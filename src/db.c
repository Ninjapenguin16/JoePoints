...
int db_init(const char* path) {
    ...
    if(sqlite3_config(SQLITE_CONFIG_SERIALIZED) != SQLITE_OK) {
        fprintf(stderr, "Warning: sqlite3_config(SQLITE_CONFIG_SERIALIZED) failed\n");
    ...
    if(sqlite3_prepare_v2(db, "SELECT name FROM sqlite_master WHERE type='table' AND name='keys';", -1, &chk, NULL) == SQLITE_OK) {
    ...
    if(!keys_exists) {
        ...
        if(sqlite3_prepare_v2(db, "PRAGMA table_info(keys);", -1, &colstmt, NULL) == SQLITE_OK) {
        ...
        if(!has_hash) {
            if(has_plain) {
                ...
                if(sqlite3_prepare_v2(db, "SELECT key FROM keys;", -1, &rstmt, NULL) == SQLITE_OK) {
                    while(sqlite3_step(rstmt) == SQLITE_ROW) {
                        ...
                        uint8_t dk[32];
                        if(pbkdf2_hmac_sha256(oldkey, strlen((const char*)oldkey), salt_raw, sizeof(salt_raw), 100000, dk, sizeof(dk)) != 0) {
                            continue;
                        }
                        char hash_hex[65];
                        ...
                        if(sqlite3_prepare_v2(db, "INSERT INTO keys_new (identifier, "
...
int db_create_key(const char* identifier, char* out_key, size_t len) {
    ...
    uint8_t dk[32];
    if(pbkdf2_hmac_sha256((const uint8_t*)out_key, strlen(out_key), salt_raw, salt_bytes, iterations, dk, sizeof(dk)) != 0) {
        return -1;
    }
    ...
    int rc = sqlite3_prepare_v2(db, "INSERT INTO keys (identifier, key_hash, salt) VALUES (?,?,?);", -1, &stmt, NULL);
    if(rc != SQLITE_OK) {
...
int db_auth_key_exists(const char* key) {
    ...
    sqlite3_stmt* stmt = NULL;
    int found = 0;
    int rc = sqlite3_prepare_v2(db, "SELECT key_hash, salt FROM keys;", -1, &stmt, NULL);
    if(rc != SQLITE_OK) {
        return 0;
    }
    const uint32_t iterations = 100000;
    while(sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        uint8_t dk[32];
        if(pbkdf2_hmac_sha256((const uint8_t*)key, strlen(key), salt_raw, salt_bytes, iterations, dk, sizeof(dk)) != 0) {
            continue;
        }
        char dk_hex[65];
...
int db_add_user(const char* first, const char* last, int* out_uid) {
    ...
    rc = sqlite3_prepare_v2(db, "SELECT COUNT(*) FROM users WHERE first=? AND last=?;", -1, &stmt, NULL);
    if(rc != SQLITE_OK) {
    ...
    rc = sqlite3_prepare_v2(db, "SELECT MIN(t1.uid + 1) "
                                "FROM users t1 "
    ...
    rc = sqlite3_prepare_v2(db, "INSERT INTO users (uid, first, last, points) VALUES (?,?,?,0);", -1, &stmt, NULL);
    if(rc != SQLITE_OK) {
        printf("[db] prepare failed: %s\n", sqlite3_errmsg(db));
...
int db_get_uid_by_name(const char* first, const char* last, int** uids, int* count) {
    sqlite3_stmt* stmt;
    if(!db) {
        return -1;
    }
    sqlite3_prepare_v2(db, "SELECT uid FROM users WHERE first=? AND last=?;", -1, &stmt, NULL);
    sqlite3_bind_text(stmt, 1, first, -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 2, last, -1, SQLITE_TRANSIENT);
...
int db_get_points(int uid, int* points) {
    ...
    sqlite3_prepare_v2(db, "SELECT points FROM users WHERE uid=?;", -1, &stmt, NULL);
    sqlite3_bind_int(stmt, 1, uid);
    int rc = -1;
    if(sqlite3_step(stmt) == SQLITE_ROW) {
...
int db_set_points(int uid, int points) {
    ...
    int rc = sqlite3_prepare_v2(db, "UPDATE users SET points=? WHERE uid=?;", -1, &stmt, NULL);
    if(rc != SQLITE_OK) {
        return -1;
    }
...
int db_add_points(int uid, int delta) {
    ...
    int rc = sqlite3_prepare_v2(db, "UPDATE users SET points=points+? WHERE uid=?;", -1, &stmt, NULL);
    if(rc != SQLITE_OK) {
        return -1;
    }
...
int db_remove_key(const char* key) {
    ...
    sqlite3_stmt* stmt = NULL;
    int rc = sqlite3_prepare_v2(db, "SELECT id, key_hash, salt FROM keys;", -1, &stmt, NULL);
    if(rc != SQLITE_OK) {
        return -1;
    }
    const uint32_t iterations = 100000;
...
int db_get_all_users(char** out_json) {
    ...
    sqlite3_stmt* stmt;
    int rc = sqlite3_prepare_v2(db, "SELECT uid,first,last,points FROM users;", -1, &stmt, NULL);
    if(rc != SQLITE_OK) {
        return -1;
    }
    ...
    while(sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        json_object_object_add(obj, "first", json_object_new_string(first ? (const char*)first : ""));
        json_object_object_add(obj, "last", json_object_new_string(last ? (const char*)last : ""));

...
void send_json_response(struct MHD_Connection *conn, int status,
                        const char *json_str) {
...
enum MHD_Result iterate_post(void *coninfo_cls, enum MHD_ValueKind kind,
  ...
  printf("[API] iterate_post called: con_info=%p off=%" PRIu64
         " size=%zu key=%s\n",
...
enum MHD_Result handle_genkey(struct MHD_Connection *conn, const char *body) {
  if (!check_auth(conn)) {
    send_json_response(conn, MHD_HTTP_FORBIDDEN,
  ...
  if (!is_content_type_json(conn)) {
    send_json_response(conn, MHD_HTTP_UNSUPPORTED_MEDIA_TYPE,
  ...
  if (!body || strlen(body) == 0) {
    send_json_response(conn, MHD_HTTP_BAD_REQUEST,
                       "{\"error\":\"Missing body (identifier required)\"}");
  ...
  if (db_create_key(identifier, key, sizeof(key)) != 0) {
    send_json_response(conn, MHD_HTTP_INTERNAL_SERVER_ERROR,
...
enum MHD_Result handle_addperson(struct MHD_Connection *conn,
  ...
  if (!first_trimmed || !last_trimmed || strlen(first_trimmed) == 0 ||
      strlen(last_trimmed) == 0) {
    send_json_response(conn, MHD_HTTP_BAD_REQUEST,
                       "{\"error\":\"Missing or empty first or last name\"}");
    free(first_trimmed);
    free(last_trimmed);
    json_object_put(json);
  ...
  if (db_add_user(first_trimmed, last_trimmed, &uid) != 0) {
    send_json_response(conn, MHD_HTTP_CONFLICT,
...
enum MHD_Result handle_getuid(struct MHD_Connection *conn) {
  ...
  if (!first_trimmed || !last_trimmed || strlen(first_trimmed) == 0 ||
      strlen(last_trimmed) == 0) {
    send_json_response(conn, MHD_HTTP_BAD_REQUEST,
                       "{\"error\":\"Missing or empty first or last\"}");
    free(first_trimmed);
    free(last_trimmed);
...
enum MHD_Result handle_getpoints(struct MHD_Connection *conn) {
  ...
  if (db_get_points(uid, &points) != 0) {
    send_json_response(conn, MHD_HTTP_NOT_FOUND,
...
enum MHD_Result handle_api_request(struct MHD_Connection *conn, const char *url,
  ...
  printf("[API] handle_api_request entry: url='%s' method='%s' con_info=%p "
         ...
         url ? url : "(null)", method ? method : "(null)", (void *)con_info,
         (void *)upload_data, (void *)upload_data_size);
  fflush(stdout);
  if (con_info) {
    printf("[API] con_info->post_data exists? %s len=%zu\n",
           con_info->post_data ? "yes" : "no",
           con_info->post_data ? strlen(con_info->post_data) : 0);
    fflush(stdout);
  }
